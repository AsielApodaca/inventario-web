...
export const usuarioController = {
  ...
  register: async (req, res, next) => {
    try {
      ...
      if (error.message.includes('requerido') || error.message.includes('debe tener') || 
          error.message.includes('Rol inválido') || error.message.includes('Ya existe')) {
  ...
  updateUsuario: async (req, res, next) => {
    try {
      ...
      const usuarioActualizado = await UsuarioService.actualizarUsuario(parseInt(id), usuarioData);
      ...
      if (error.message.includes('inválido') || error.message.includes('no encontrado') || 
          error.message.includes('vacío') || error.message.includes('Rol inválido') || 
          error.message.includes('Ya existe') || error.message.includes('debe tener')) {
        const statusCode = error.message.includes('no encontrado') ? 404 : 400;
        return res.status(statusCode).json({
          status: 'error',
          message: error.message
  ...
  deleteUsuario: async (req, res, next) => {
    try {
      ...
      const resultado = await UsuarioService.eliminarUsuario(parseInt(id));
